...
namespace Graph {
    ...
    public:
		...
		const std::set<std::string>& Deleted() const { return deleted; } // 获取已删除的地点
		...
		void UpdateEdge(const std::string& vertex_x_name, const std::string& vertex_y_name, GElemSet new_weight); // 修改边的权重
